...
export const VoiceOrderModal: React.FC<VoiceOrderModalProps> = ({
  ...
  // Quick Controls State within Voice Modal - ALWAYS VISIBLE
  ...
  // Client selection / quick registration state - ALWAYS VISIBLE
  ...
  const [position, setPosition] = useState<{ x: number; y: number }>({ x: 16, y: 55 });
  const isDraggingRef = useRef(false);
  const dragStartOffsetRef = useRef<{ x: number; y: number }>({ x: 0, y: 0 });

  useEffect(() => {
    if (isOpen) {
      const initialY = Math.max(50, Math.min(window.innerHeight - 720, 55));
      const initialX = Math.max(8, Math.min(window.innerWidth - 740, 16));
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const currentWidth = isMinimized ? 340 : (isWide ? 760 : 620);
    const newX = Math.max(4, Math.min(window.innerWidth - currentWidth, e.clientX - dragStartOffsetRef.current.x));
    const newY = Math.max(45, Math.min(window.innerHeight - 80, e.clientY - dragStartOffsetRef.current.y));
  ...
  const POPULAR_SUGGESTIONS = [
    { name: '1 Metro Muzzarella + 2 Fainás', price: 750, category: 'promos', icon: '⭐', badge: 'Promo $750' },
    { name: '1 Metro Muzzarella + 2 Fainás + Refresco 1.5L', price: 900, category: 'promos', icon: '🥤', badge: 'Promo $900' },
    { name: '1 Metro Muzzarella + 2 Fainás + 1 Chajá', price: 920, category: 'promos', icon: '🍰', badge: 'Promo $920' },
    { name: '1 Metro Muzzarella + 2 Fainás + 2 Flanes', price: 1000, category: 'promos', icon: '🍮', badge: 'Promo $1000' },
    { name: 'Fainá con Queso', price: 180, category: 'fainas', icon: '🧀' },
    { name: 'Fainá común', price: 130, category: 'fainas', icon: '🔥' },
    { name: 'Sándwich caliente', price: 350, category: 'sandwiches', icon: '🥪' },
  ];
    ...
    <div className="fixed inset-0 z-50 pointer-events-none">
      <div
        ...
        style={{
          ...
          width: isMinimized ? '340px' : (isWide ? '760px' : '620px'),
          maxWidth: 'calc(100vw - 16px)',
        }}
        className="pointer-events-auto absolute bg-[#05080c] border-2 border-purple-500/50 rounded-3xl shadow-2xl overflow-hidden flex flex-col text-slate-100 transition-all duration-150 backdrop-blur-2xl max-h-[92vh]"
      >
        {/* Draggable Header */}
        <div
        ...
        >
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              title={isWide ? 'Vista normal (620px)' : 'Vista amplia (760px)'}
            >
        ...
        {isMinimized ? (
          ...
          <div className="flex-1 flex flex-col min-h-0 overflow-hidden bg-[#06030c]">
            <div className="p-3.5 sm:p-4 space-y-3 overflow-y-auto no-scrollbar flex-1">
              ...
              {/* TOP STRIP: Live Listening Controls + Transcript */}
              <div className="bg-gradient-to-r from-[#120726] to-[#0a0316] p-3 rounded-2xl border border-purple-500/30 space-y-2 shadow-inner">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <button
                      type="button"
                      onClick={toggleListening}
                      className={`w-10 h-10 rounded-2xl flex items-center justify-center transition-all shrink-0 shadow-lg cursor-pointer ${
                        isLiveListening
                    ...
                    >
                      <Icon name={isLiveListening ? "mic" : "mic_off"} size={20} />
                    </button>

                    <div>
                      <div className="text-xs font-black uppercase text-white flex items-center gap-1.5">
                        <span>{isLiveListening ? 'Dictado continuo en vivo' : 'Dictado pausado'}</span>
                      </div>
                      <div className="text-[10px] text-purple-300/80 font-medium">
                        {isLiveListening ? 'Habla y detecta promos, productos, destino y pago.' : 'Toca el micrófono para comenzar o reanudar.'}
                      </div>
                    </div>
                  </div>
                  ...
                  {transcript && (
                    <button
                      ...
                      className="text-[10px] text-slate-400 hover:text-red-400 flex items-center gap-1 font-bold cursor-pointer"
                    >
                ...
                <textarea
                  ...
                  placeholder='Dicta ej: "promo 750, 2 fainá con queso, para enviar a Juan en 18 de julio paga con 1000"'
                  rows={2}
                  className="w-full bg-[#040108] border border-purple-500/30 focus:border-purple-400 rounded-xl p-2 text-xs text-purple-100 placeholder-slate-600 focus:outline-none transition-all resize-none shadow-inner font-medium"
                />
              </div>

              {micError && (
                <div className="p-2 bg-red-950/60 border border-red-500/40 rounded-xl text-[11px] text-red-200 flex items-center gap-2">
                  <Icon name="warning" size={16} className="text-red-400 shrink-0" />
                  <span>{micError}</span>
                </div>
              )}

              {/* SECTION: DESTINO + PAGO + CLIENTE RÁPIDO (ALWAYS VISIBLE) */}
              <div className="bg-[#0a0416] border border-purple-500/30 rounded-2xl p-3 space-y-2.5 shadow-lg text-left">
                <div className="text-[11px] font-black uppercase text-purple-300 flex items-center gap-1.5 border-b border-purple-500/20 pb-1.5">
                  <Icon name="tune" size={15} className="text-purple-400" />
                  <span>1. Destino, Método de Pago & Cliente Rápido</span>
                </div>
                        ...
                        <div className="flex gap-1 overflow-x-auto no-scrollbar py-0.5">
                          {Array.from({ length: 20 }, (_, i) => i + 1).map(num => (
                            <button
                              ...
                              className={`w-6 h-6 rounded-md text-[11px] font-black font-mono shrink-0 transition-all border cursor-pointer ${
                    ...
                    <div className="grid grid-cols-5 gap-1">
                      {['Efectivo', 'Débito', 'Crédito', 'Transferencia', 'Mercado Pago'].map(pm => (
                        <button
                          ...
                          className={`p-1.5 rounded-lg text-[10px] font-black uppercase border transition-all cursor-pointer text-center truncate ${
                    ...
                    {quickPaymentMethod === 'Efectivo' && (
                      <div className="flex items-center gap-2 pt-0.5">
                        ...
                        <input
                          ...
                          className="flex-1 p-1 bg-[#06020e] border border-emerald-500/40 text-emerald-300 rounded-lg text-xs font-mono font-black outline-none focus:border-emerald-400 text-center"
                        />
                      </div>
                    )}
                  </div>
                ...
                {/* 3. Selector y Registro de Cliente */}
                <div className="space-y-1 relative pt-1 border-t border-purple-500/20">
                  <div className="flex items-center justify-between">
                    <label className="text-[10px] font-black uppercase text-slate-400">
                      Cliente (Buscar en Directorio o Registrar Nuevo)
                    </label>
                    {selectedClient && (
                      <button
                        type="button"
                      ...
                      >
                        Quitar selección ({selectedClient.name})
                      </button>
                    )}
                  </div>
                  ...
                  <div className="relative">
                    <input
                      type="text"
                      placeholder="Buscar cliente registrado en directorio (nombre o cel)..."
                      ...
                      className="w-full p-1.5 bg-[#06020e] border border-purple-500/30 text-white rounded-xl text-xs font-black uppercase outline-none focus:border-purple-400"
                    />
                  </div>
              ...
              {/* PROMOS Y LO MÁS PEDIDO (1-CLICK SHORTCUTS) */}
              <div>
                <div className="flex items-center justify-between text-[10px] font-black uppercase text-slate-400 mb-1 px-1">
                  <span className="flex items-center gap-1 text-purple-400">
                    <Icon name="trending_up" size={13} /> Atajos Rápidos de Promos & Menú
                  </span>
                  <span className="text-[9px] text-slate-500">Toca para agregar</span>
                </div>
                <div className="flex items-center gap-1.5 overflow-x-auto pb-1 no-scrollbar">
                  {POPULAR_SUGGESTIONS.map(sugg => (
                    <button
                      ...
                      className="px-2.5 py-1.5 bg-[#0e0620] hover:bg-purple-950/90 border border-purple-500/40 hover:border-purple-400 text-purple-200 hover:text-white rounded-xl text-[10px] font-black uppercase transition-all shrink-0 flex items-center gap-1.5 cursor-pointer shadow-sm"
                      title={`Agregar 1x ${sugg.name} ($${sugg.price})`}
                    >
                      <span>{sugg.icon}</span>
                      <span>{sugg.badge || sugg.name}</span>
                      <span className="text-emerald-400 font-mono text-[9px] font-black">${sugg.price}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-[#090314] border-2 border-purple-500/40 rounded-2xl p-3 space-y-2 shadow-xl">
                <div className="flex items-center justify-between border-b border-purple-500/25 pb-1.5">
                ...
                {(!parsedResult || parsedResult.items.length === 0) ? (
                  <div className="text-center py-3 text-xs font-bold text-slate-400 bg-[#06020e] border border-purple-500/15 rounded-xl p-2.5">
                    {transcript.trim() 
                      ? 'Reconociendo frase... Dicta "promo 750", "promo 900", "2 fainás", etc.' 
                      : '🎙️ Dicta productos o promos por voz, o toca los botones de arriba para agregarlos a la comanda.'}
                  </div>
                ) : (
            ...
            {parsedResult && parsedResult.items.length > 0 && (
              <div className="p-3 bg-[#0a0416] border-t border-purple-500/30 grid grid-cols-2 gap-2 shrink-0 shadow-2xl">
                <button
                  type="button"
                  onClick={() => handleApplyBatchAndContinue(false)}
                  className="py-3 px-3 bg-[#16082c] hover:bg-[#230d45] border border-purple-500/50 text-purple-200 font-black text-xs uppercase rounded-2xl transition-all flex items-center justify-center gap-2 shadow-lg cursor-pointer"
                >
                ...
                <button
                  ...
                  className="py-3 px-3 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-black text-xs uppercase rounded-2xl transition-all flex items-center justify-center gap-2 shadow-lg shadow-purple-600/40 cursor-pointer"
                >
